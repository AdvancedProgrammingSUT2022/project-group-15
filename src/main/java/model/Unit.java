...
public abstract class Unit {
    ...
    public void move(int x, int y) {
        PlanedToGo = findShortestPathByDijkstra(x, y);
    ...
    private ArrayList<Hex> findShortestPathByDijkstra(int x, int y) {
    ...
    private boolean validCoordinate(int x, int y) {
        if (x < 0 || y < 0)
            return false;
        if (x > Game.getGame().getRows() || y > Game.getGame().getColumns())
            return false;
        return true;
    }
